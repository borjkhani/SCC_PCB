...
% 
 ...
 writerObj = VideoWriter('A','MPEG-4');
 writerObj.FrameRate = 1; %3 VIP % 1 Slow 60 Fast
 open(writerObj);
% 
 F1=figure;
skipNum = [86,510]; %[24,506,PV,Sst];
% pause (2)
for Cell_Number = PYRs %Sst %VIP %Sst %PVs %1:10 %PYRs %PVs %1:510 % Number of Cells to be Analysed  334
...
%run Function_Slow_Plot_All_In_One.m %('C:\Users\Mehdi\Downloads\Model 8\Matlab Analysis Andrzej\Run_One_PYRs_Just_PV.m')
%close all
clc
...
legend off;
set(1,'renderer','painters')
saveas(F1,sprintf('Z_%s %d All N.tif',Ce,Cell_Number));
saveas(F1,sprintf('Z_%s %d All N.fig',Ce,Cell_Number));
saveas(F1,sprintf('Z_%s %d All N.eps',Ce,Cell_Number),'psc2');
